...
public class SeamDependenciesDataModel extends GraphDataModel<SeamComponentNodeInfo, SeamDependencyInfo> {
  ...
  private final Collection<SeamComponentNodeInfo> myNodes = new HashSet<SeamComponentNodeInfo>();
  private final Collection<SeamDependencyInfo> myEdges = new HashSet<SeamDependencyInfo>();
  ...
  public void updateDataModel() {
    ...
    Map<String, SeamJamComponentNodeInfo> annotatedNodesMap = new HashMap<String, SeamJamComponentNodeInfo>();
    Map<String, SeamJamComponentNodeInfo> compiledNodesMap = new HashMap<String, SeamJamComponentNodeInfo>();
    Map<String, SeamDomComponentNodeInfo> domNodesMap = new HashMap<String, SeamDomComponentNodeInfo>();
    Map<String, UnknownBijectionNodeInfo> unknownNodesMap = new HashMap<String, UnknownBijectionNodeInfo>();

    List<Pair<String, PsiType>> jamComponentsInfo = new ArrayList<Pair<String, PsiType>>();
    ...
    for (SeamJamComponent component : seamJamComponents) {
      ...
      if (!StringUtil.isEmptyOrSpaces(componentName)) {
        ...
        jamComponentsInfo.add(new Pair<String, PsiType>(componentName, component.getComponentType()));
      }
    }

    Map<String, SeamDomFactory> domFactoriesMap = new HashMap<String, SeamDomFactory>();

    for (SeamDomModel model : SeamDomModelManager.getInstance(myModule.getProject()).getAllModels(myModule)) {
      for (SeamDomComponent seamDomComponent : model.getSeamComponents()) {
        ...
        if (!StringUtil.isEmptyOrSpaces(name) && !jamComponentsInfo.contains(new Pair<String, PsiType>(name, type))) {
  ...
  private SeamComponentNodeInfo findNodeByName(@NotNull String name,
                                               ...
                                               Map<String, SeamDomFactory> domFactoriesMap) {
       return findNodeByName(name, nodesMap, domNodesMap, compiledNodesMap,domFactoriesMap, new HashSet<String>());
  }
  @Nullable
  private SeamComponentNodeInfo findNodeByName(@NotNull String name,
                                               Map<String, SeamJamComponentNodeInfo> nodesMap,
