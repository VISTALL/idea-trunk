...
public class SeamAnnotationsInconsistencyInspection extends SeamJamModelInspectionBase {
  ...
  private static boolean isDefinedInXml(final PsiClass aClass) {
    ...
    if (module != null) {
      for (SeamDomModel seamDomModel : SeamDomModelManager.getInstance(aClass.getProject()).getAllModels(module)) {
          if (seamDomModel.getSeamComponents(JavaPsiFacade.getInstance(module.getProject()).getElementFactory().createType(aClass)).size() > 0) return true;
      }
    }
    return false;
  }
  ...
  public String getDisplayName() {
    return SeamInspectionBundle.message("jam.annotation.inconsistency.inspection.name");
  }
