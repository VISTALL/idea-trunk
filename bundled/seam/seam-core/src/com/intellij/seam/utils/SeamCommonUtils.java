...
public class SeamCommonUtils {
  ...
  public static Set<ContextVariable> getSeamContextVariables(@Nullable final Module module, boolean jam, boolean dom) {
    return getSeamContextVariables(module, jam, dom, true);
  }
  ...
  public static Set<ContextVariable> getSeamContextVariables(@Nullable final Module module, boolean jam, boolean dom, boolean fromLibs) {
    Set<ContextVariable> vars = new HashSet<ContextVariable>();
    if (module != null) {
      ...
      if (dom) {
        ...
        addXmlDefinedFactoryContextVariables(module, globalImports, vars);
        addMergedContextVariables(module, vars, fromLibs);
      }

      Set<ContextVariable> notImported = new HashSet<ContextVariable>(vars);
  ...
  public static List<String> getSeamContextVariableNames(final Module module) {
    return ContainerUtil.map2List(getSeamContextVariablesWithDependencies(module), new Function<ContextVariable, String>() {
      public String fun(final ContextVariable contextVariable) {
        return contextVariable.getName();
      }
    });
  }

  public static void addAnnotatedContextVariables(final Module module, final Set<ContextVariable> vars, final boolean fromLibs) {
    final Set<SeamJamComponent> seamComponents = SeamJamModel.getModel(module).getSeamComponents(false, fromLibs);

    final Set<ContextVariable> annotatedVariables = new HashSet<ContextVariable>();
  ...
  private static boolean isAlreadyDefined(@NotNull final String name, final Set<ContextVariable> vars) {
  ...
  private static boolean isAlreadyDefined(@NotNull final String name, @NotNull final PsiType type, final Set<ContextVariable> vars) {
    for (ContextVariable var : vars) {
  ...
  private static void addBundleContextVariables(final Set<ContextVariable> vars, final Project project, final SeamDomModel model) {
    for (DomFileElement<SeamComponents> fileElement : model.getRoots()) {
      if (!fileElement.isValid()) continue;
      for (BasicBundleNamesHolder bundleNamesHolder : DomUtil
        .getDefinedChildrenOfType(fileElement.getRootElement(), BasicBundleNamesHolder.class)) {
  ...
  private static void addNotNullContextVariable(@Nullable final ContextVariable contextVariable, @NotNull final Set<ContextVariable> vars) {
  ...
  public static Set<SeamFacet> getAllSeamFacets(final Project project) {
    final Set<SeamFacet> result = new HashSet<SeamFacet>();
  ...
  public static Set<SeamFacet> getAllSeamFacetsWithDependencies(final Module module) {
    CachedValue<Set<SeamFacet>> cachedValue = module.getUserData(MODULE_SEAM_FACETS);
    if (cachedValue == null) {
      cachedValue =
        PsiManager.getInstance(module.getProject()).getCachedValuesManager().createCachedValue(new CachedValueProvider<Set<SeamFacet>>() {
          public Result<Set<SeamFacet>> compute() {
            final Set<SeamFacet> facets = new HashSet<SeamFacet>();
            for (Module depModule : JamCommonUtil.getAllDependentModules(module)) {
              facets.addAll(getAllSeamFacets(depModule));
            }
            return new Result<Set<SeamFacet>>(facets, ProjectRootManager.getInstance(module.getProject()));
          }
        }, false);
  ...
  // returns all dependent modules where SeamFacet doesn't defined
  @NotNull
  public static Set<Module> getDependentFacetModules(final SeamFacet facet) {
    Set<Module> modules = new HashSet<Module>();
    final Module module = facet.getModule();
    for (Module depModule : JamCommonUtil.getAllDependentModules(module)) {
      if (!isModuleContainsSeamFacet(depModule)) {
  ...
  public static boolean isSeamFacetDefined(@Nullable final Module module) {
    ...
    if (isModuleContainsSeamFacet(module)) return true;

    // check module dependencies
    for (Module depModule : JamCommonUtil.getAllModuleDependencies(module)) {
      if (isModuleContainsSeamFacet(depModule)) return true;
    }
  ...
  public static List<CommonModelElement> findSeamComponents(final String name, final Module module) {
    List<CommonModelElement> components = new ArrayList<CommonModelElement>();
  ...
  public static PsiType getUnwrapType(final PsiClass psiClass) {
    for (PsiMethod method : psiClass.getMethods()) {
      if (AnnotationUtil.isAnnotated(method, SeamAnnotationConstants.UNWRAP_ANNOTATION, false)) {
  ...
  public static SeamJamComponent getSeamJamComponent(PsiClass aClass) {
    return JamService.getJamService(aClass.getProject()).getJamElement(SeamJamComponent.class, aClass);
  }
    ...
    if (type instanceof PsiClassType) {
      ...
      if (psiClass != null) {
        ...
        if (seamComponent != null) {
          ...
          if (componentName != null && componentName.equals(domComponent.getComponentName())) {
  ...
  public static FileTemplate chooseTemplate(final Module module) {
    ...
    return FileTemplateManager.getInstance().getJ2eeTemplate(templateName);
  }
