...
public abstract class MobileModuleSettings implements JDOMExternalizable {
  ...
  protected HashSet<UserDefinedOption> userDefinedOptions = new HashSet<UserDefinedOption>();
  ...
  public void readSetting(Element parentElement) {
    for (Iterator<Element> iterator = parentElement.getChildren(SETTING).iterator(); iterator.hasNext();) {
  ...
  protected void putIfNotExists(String key, String value) {
    if (value != null && key != null) {
      if (properties.containsKey(key) && (properties.get(key) == null || properties.get(key).length() == 0)) {
        properties.put(key, value);
      }
  ...
  public void initSettings(J2MEModuleBuilder moduleBuilder) {
    if (moduleBuilder.getModuleFileDirectory() != null) {
      if (!myDefaultModified || myJarURL == null || myJarURL.length() == 0) {
      ...
      if (!myDefaultModified || myMobileDescriptionPath == null || myMobileDescriptionPath.length() == 0) {
  ...
  public void prepareJarSettings() {
    ...
    if (myJarURL != null) {
      properties.put(mobileApplicationType.getJarSizeSettingName(), new Long(new File(myJarURL).length()).toString());
    }
  }
