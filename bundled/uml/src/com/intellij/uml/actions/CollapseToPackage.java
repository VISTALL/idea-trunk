 ...
 */
public class CollapseToPackage extends UmlAction {
  @Override
  ...
  public void actionPerformed(final AnActionEvent e) {
    ...
    ProgressManager.getInstance().runProcessWithProgressSynchronously(new Runnable() {
      public void run() {
        ...
        List<Node> selectedNodes = GraphViewUtil.getSelectedNodes(graph);
        List<UmlNode> packages = new ArrayList<UmlNode>();
        for (Node node : selectedNodes) {
          UmlNode umlNode = builder.getNodeObject(node);

          if (umlNode == null) continue;
          if (umlNode.getIdentifyingElement() instanceof PsiPackage) {
            packages.add(umlNode);
            continue;
          }

          UmlUtils.getDataModel(builder).collapseToPackage(umlNode);
        }
        if (packages.size() == selectedNodes.size()) {
          for (UmlNode umlNode : packages) {
            UmlUtils.getDataModel(builder).collapseToPackage(umlNode);
          }
        }

        UmlUtils.updateGraph(builder);
      }
    },
