 ...
 */
public class AddFieldAction extends UmlAction {
  @Override
  public void update(final AnActionEvent e) {
    ...
    e.getPresentation().setEnabled(psiElement instanceof PsiClass);
  }

  public void actionPerformed(final AnActionEvent e) {
    final GraphBuilder<UmlNode, UmlEdge> builder = getBuilder(e);
    final PsiClass psiClass = (PsiClass)DataKeys.PSI_ELEMENT.getData(e.getDataContext());
    if (builder == null || psiClass == null) return;

    if (! prepareClassForWrite(psiClass)) return;

    try {
      final DialogWrapper dialog = new CreateNewFieldDialog(psiClass);
