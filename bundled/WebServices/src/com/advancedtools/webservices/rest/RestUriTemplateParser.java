 ...
 */
public class RestUriTemplateParser {
   ...
   */
  public RestUriTemplateParser(String template) {
    if (template == null || template.length() == 0) throw new IllegalArgumentException("Empty path");
  ...
  private void parse(CharacterIterator ci) {
    try {
      ...
      throw new IllegalArgumentException(
        "Invalid syntax for the template, \"" + template + "\". Check if a path parameter is terminated with a '}'.", ex);
    }
  }
