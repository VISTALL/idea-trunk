...
/**
 * Non JApplet port of Axis's SoapMonitorApplet
 */
public class AxisSOAPMonitor {
  ...
  private java.util.List<SOAPMonitorPage> pages = null;
  ...
  public AxisSOAPMonitor() {
    ...
    pages = new ArrayList<SOAPMonitorPage>(1);
  }

  public boolean hasPageWithPort(int port) {
    for (SOAPMonitorPage page : pages) {
      if (page.getPort() == port) return true;
    }
  ...
  public void addPage(int port) {
    if (hasPageWithPort(port)) return;
  ...
  class SOAPMonitorPage extends JPanel
    ...
    private final EmptyBorder empty_border = null;
     ...
     */
    public SOAPMonitorPage(String host_name, int _port) {
      host = host_name;
      port = _port;
  ...
  class ServiceFilterPanel extends JPanel
    ...
    private final Vector filter_list = null;
