...
public class AddStrutsSupportUtil {
  ...
  private static PsiFile createConfigFile(final Project project, final VirtualFile webinf, final boolean isStruts13x) throws Exception {
    ...
    final FileTemplate fileTemplate = FileTemplateManager.getInstance().getJ2eeTemplate(StrutsFileTemplateGroupFactory.STRUTS_CONFIG_XML);
  ...
  private static VirtualFile getOrCreateWebInf(final WebFacet webFacet) throws IOException {
    ...
    if (webinf == null) {
      ...
      if (parent != null && parent.getName().equals(WEB_INF)) {
        ...
        WebRoot root;
        List webRoots = webFacet.getWebRoots(false);
        if (webRoots.isEmpty()) {
  ...
  private static void addTilesSupport(final @NotNull PsiDirectory webinf, @NotNull final StrutsConfig root) throws Exception {
    ...
    if (tilesDefs == null) {
      final FileTemplate fileTemplate = FileTemplateManager.getInstance().getJ2eeTemplate(StrutsFileTemplateGroupFactory.TILES_DEFS_XML);
  ...
  private static void copyValidatorRulesAndCreateValidation(@NotNull final PsiDirectory webinf) throws Exception {
    ...
    if (rulesXml == null) {
      final FileTemplate fileTemplate = FileTemplateManager.getInstance().getJ2eeTemplate(StrutsFileTemplateGroupFactory.VALIDATOR_RULES_XML);
    ...
    if (roots.length > 0) {
      ...
      if (directory != null) {
        ...
        final FileTemplate fileTemplate =
          FileTemplateManager.getInstance().getJ2eeTemplate(StrutsFileTemplateGroupFactory.MESSAGE_RESOURCES_PROPERTIES);
  ...
  private static void createValidationXml(final String v, @NotNull PsiDirectory directory) throws Exception {
    ...
    final FileTemplate fileTemplate = FileTemplateManager.getInstance().getJ2eeTemplate(StrutsFileTemplateGroupFactory.VALIDATION_XML);
    final Properties properties = FileTemplateManager.getInstance().getDefaultProperties();
  ...
  public static void addSupportInWriteCommandAction(final WebFacet webFacet, final boolean hasValidatorSupport, final boolean hasTilesSupport,
    ...
    new WriteCommandAction.Simple(webFacet.getModule().getProject(), getStrutsFiles(webFacet)) {
