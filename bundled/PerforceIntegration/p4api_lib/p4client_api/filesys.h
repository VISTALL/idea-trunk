...
class FileSys {
	...
	void		MkDir( const StrPtr &p, Error *e );
	void		MkDir( Error *e ) { MkDir( path, e ); }
	void		RmDir( const StrPtr &p, Error *e );
...
# endif
	...
	static void	SetCharSet( int x = 0 ) { globCharSet = x; }
	static int	GetCharSet() { return globCharSet; }
