...
public final class ExecResult {
  ...
  public void allowSafeStdoutUsage(final ThrowableConsumer<InputStream, IOException> consumer) throws IOException {
    if (myOutputGobbler != null) {
      ...
      consumer.consume(new ByteArrayInputStream(myStdout.getBytes()));
    }
  }
  ...
  private static String readStreamConvertingLineSeparators(final StreamGobbler gobbler) {
    try {
      ...
      gobbler.allowSafeStreamUsage(new ThrowableConsumer<InputStream, IOException>() {
  ...
  public byte[] getByteOut() {
    final Ref<byte[]> ref = new Ref<byte[]>();
    if (myOutputGobbler != null) {
      try {
        myOutputGobbler.allowSafeStreamUsage(new ThrowableConsumer<InputStream, IOException>() {
          public void consume(final InputStream inputStream) throws IOException {
            ref.set(FileUtil.loadBytes(inputStream, myOutputGobbler.getResultLength()));
          }
        });
