...
public class OutputMessageParser {
  protected List<String> myLines = new ArrayList<String>();
  ...
  private static class ChangeReadingPolicy {
    ...
    public ChangeReadingPolicy(final Pattern pattern, final int userGroup, final int dateGroupGroup) {
      myPattern = pattern;
  ...
  protected OutputMessageParser(final String output) throws IOException {
  ...
  public static List<P4Revision> processLogOutput(String output, final boolean newDateFormat) throws IOException {
  ...
  private P4Revision readNextRevision(final boolean newDateFormat) throws ParseException {
    ...
    SyncDateFormat format = newDateFormat ? NEW_DATE_FORMAT : OLD_DATE_FORMAT;
    if (logMatcher.matches()) {
      final P4Revision result =
        new P4Revision(myDepotPath, Long.parseLong(logMatcher.group(2)), Long.parseLong(logMatcher.group(4)),logMatcher.group(6),
                       format.parse(logMatcher.group(8)), logMatcher.group(10), logMatcher.group(12), logMatcher.group(14),
                       myIsBranched);
  ...
  public static List<ChangeListData> processChangesOutput(final String output) throws IOException {
    final ArrayList<ChangeListData> result = new ArrayList<ChangeListData>();
    final OutputMessageParser parser = new OutputMessageParser(output);
  ...
  public static List<String> processClientsOutput(final String output) throws IOException {
  ...
  public static List<String> processUsersOutput(final String output) throws IOException {
    final ArrayList<String> result = new ArrayList<String>();
  ...
  public static List<PerforceAbstractChange> processDescriptionOutput(final String output) throws IOException {
    final ArrayList<PerforceAbstractChange> result = new ArrayList<PerforceAbstractChange>();

    final OutputMessageParser parser = new OutputMessageParser(output);

    while (!parser.myLines.isEmpty()) {
      parser.myCurrentLine = parser.myLines.remove(0);
      if (parser.myCurrentLine.startsWith(AFFECTED_FILES_PREFIX)) {
        break;
      }
    }
    ...
    while (!parser.myLines.isEmpty()) {
      parser.myCurrentLine = parser.myLines.remove(0);
      if (parser.myCurrentLine.startsWith(FILE_PREFIX)) {
        final FileChange fileChange = parser.createFileChange();
        if (fileChange != null) {
...
*/
  ...
  private FileChange createFileChange() {
    ...
    return new FileChange(repositoryFilePath, file, Long.parseLong(fileRev), type);
  }

  public static List<String> processBranchesOutput(final String output) throws IOException {
    final ArrayList<String> result = new ArrayList<String>();
  ...
  @Nullable
  public static ChangeListData loadChangeListDescription(final String output) throws IOException {
    return new OutputMessageParser(output).readNextChange(new ChangeReadingPolicy(ChangeReadingPolicy.CHANGE_DESCRIPTION_PATTERN,4, 8));
  }
}
