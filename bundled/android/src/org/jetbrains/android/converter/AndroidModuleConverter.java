...
public class AndroidModuleConverter extends ConversionProcessor<ModuleSettings> {
  ...
  private static Element findAndroidFacetConfigurationElement(ModuleSettings moduleSettings) {
    final Collection<? extends Element> facetElements = moduleSettings.getFacetElements(AndroidFacet.getFacetType().getStringId());
    if (facetElements.isEmpty()) return null;
  ...
  @Nullable
  private static Library findAppropriateAndroidPlatform(IAndroidTarget target, AndroidSdk sdk) {
  ...
  private static Library getAndroidPlatform(String sdkPath, @Nullable String targetName) {
    ...
    if (sdk != null) {
      if (targetName == null) {
        targetName = sdk.getDefaultTargetName();
      }
      IAndroidTarget target = sdk.findTargetByName(targetName);
      if (target != null) {
        Library library = findAppropriateAndroidPlatform(target, sdk);
        if (library == null) {
